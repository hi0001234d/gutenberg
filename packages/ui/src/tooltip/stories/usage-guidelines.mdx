...
-   **Prefer using tooltips as visual labels only.** Tooltips should act as
    ...
    Tooltips alone are not accessible to touch or screen reader users.
    See _Alternatives to tooltips_ below for more details.
-   **Provide an accessible name for the trigger.** Tooltips are visual-only
    ...
    tooltip's trigger must have an `aria-label` attribute that closely
    matches the tooltip's content to ensure consistency for screen reader
    users.
...
Tooltips are designed for sighted users and are not a reliable way to deliver
...
content, such as a snackbar or notice.
